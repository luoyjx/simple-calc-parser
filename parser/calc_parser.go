...
type ManulDivContext struct {
	*ExpressionContext
	op antlr.Token
}

func NewManulDivContext(parser antlr.Parser, ctx antlr.ParserRuleContext) *ManulDivContext {
	var p = new(ManulDivContext)
...
func (s *ManulDivContext) GetOp() antlr.Token { return s.op }

func (s *ManulDivContext) SetOp(v antlr.Token) { s.op = v }

func (s *ManulDivContext) GetRuleContext() antlr.RuleContext {
	return s
}

func (s *ManulDivContext) AllExpression() []IExpressionContext {
	var ts = s.GetTypedRuleContexts(reflect.TypeOf((*IExpressionContext)(nil)).Elem())
	var tst = make([]IExpressionContext, len(ts))
...
func (s *ManulDivContext) Expression(i int) IExpressionContext {
	var t = s.GetTypedRuleContext(reflect.TypeOf((*IExpressionContext)(nil)).Elem(), i)
...
func (s *ManulDivContext) MUL() antlr.TerminalNode {
	return s.GetToken(CalcParserMUL, 0)
}

func (s *ManulDivContext) DIV() antlr.TerminalNode {
	return s.GetToken(CalcParserDIV, 0)
}

func (s *ManulDivContext) EnterRule(listener antlr.ParseTreeListener) {
	if listenerT, ok := listener.(CalcListener); ok {
		listenerT.EnterManulDiv(s)
	}
}

func (s *ManulDivContext) ExitRule(listener antlr.ParseTreeListener) {
	if listenerT, ok := listener.(CalcListener); ok {
		listenerT.ExitManulDiv(s)
	}
}
...
func (p *CalcParser) expression(_p int) (localctx IExpressionContext) {
	...
	for _alt != 2 && _alt != antlr.ATNInvalidAltNumber {
		if _alt == 1 {
			...
			case 1:
				localctx = NewManulDivContext(p, NewExpressionContext(p, _parentctx, _parentState))
				...
				}
				{
					...
					localctx.(*ManulDivContext).op = _lt
					...
					if !(_la == CalcParserMUL || _la == CalcParserDIV) {
						...
						localctx.(*ManulDivContext).op = _ri
					} else {
